...
def profile():
    """
    This command allow to config modules APIM and AM
...
def load(obj, profile_name):
    ...
    click.echo("Switch env from {} to {}".format(old_profile, profile_name))


profile.add_command(get)
profile.add_command(set)
profile.add_command(load)
