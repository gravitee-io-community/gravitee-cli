...
class ApiSchema:
    ...
    # def get_api_id(self):
    #     file_id = "{}/_ids".format(self.settings_folder)
    #     try:
    #         with open(file_id, 'r') as f:
    #             api_name_to_id = yaml.safe_load(f)
    #     except FileNotFoundError:
    #         raise GraviteeioError("Missing file {}".format(file_id))
    #     return api_name_to_id[self.api_name]
    
    # def save_api_id(self, id):
    #     file_id = "{}/_ids".format(self.settings_folder)
    #     if os.path.exists(file_id):
    #         raise GraviteeioError("API name {} already exists".format(file_id))
        
    #     with open(file_id, 'w+') as f:
    #         api_name_to_id = yaml.safe_load(f)
    #         if self.api_name in api_name_to_id:
    #             raise GraviteeioError("API name {} already set".format(file_id))
    #         api_name_to_id[self.api_name]=id
    #         yaml.dump(api_name_to_id, f)

    # def _enum_validation(self, api):

    #     for rule in enum_validation_rules:
    #         values_enum_found = jmespath.search(rule['given'], api)

    #         if values_enum_found and 'then' in rule and 'in' in rule['then']:

    #             if values_enum_found and type(values_enum_found) is list:
    #                 for value in values_enum_found:
    #                     self._validator(value, rule)
    #             else:
    #                 self._validator(values_enum_found, rule)

                    
    # def _validator(self,value, rule):
    #     if value.upper() not in rule['then']['in']:
    #                     raise TypeError(
    #                         "Got an unexpected keyword argument '{}'"
    #                         " for {} in API definition".format(value, rule['type'])
    #                     )


        # enum_loadballancer = ['ROUND_ROBIN','RANDOM','WEIGHTED_ROUND_ROBIN','WEIGHTED_RANDOM']
        # # contains(proxy.groups[*].load_balancing.type,`ROUND_ROBIN`)
